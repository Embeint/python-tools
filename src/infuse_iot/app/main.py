...
class InfuseApp:
    ...
    def _load_tools(self, parser: argparse.ArgumentParser):
        ...
        for _, name, _ in pkgutil.walk_packages(infuse_iot.tools.__path__):
            ...
            # Add tool to parser
            tool_cls : InfuseCommand = getattr(module, name)
            parser = tools_parser.add_parser(name, help=tool_cls.HELP, description=tool_cls.DESCRIPTION, formatter_class=argparse.RawDescriptionHelpFormatter)
            parser.set_defaults(tool_class=tool_cls)
            tool_cls.add_parser(tool_cls, parser)
