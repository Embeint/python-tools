...
class rpc(InfuseCommand):
    ...
    def add_parser(cls, parser):
        ...
        for _, name, _ in pkgutil.walk_packages(wrappers.__path__):
            ...
            cmd_parser = command_list_parser.add_parser(name, help=cmd_cls.HELP, description=cmd_cls.DESCRIPTION, formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    def run(self):
        ...
        pkt = {
            "pkt_type": data_types.RPC_CMD,
            "raw": request_packet.hex(),
        }
        self._client.send(pkt)
        # Wait for responses
        while rsp := self._client.receive():
            # RPC response packet
            if rsp['pkt_type'] != data_types.RPC_RSP:
                continue
            rsp_bytes = bytes.fromhex(rsp['raw'])
            rsp_header = self.rpc_response_header.from_buffer_copy(rsp_bytes)
            # Response to the request we sent
            if rsp_header.request_id != self._request_id:
                continue
            # Convert response bytes back to struct form
            rsp_data = self._command.response.from_buffer_copy(rsp_bytes[ctypes.sizeof(self.rpc_response_header):])
            # Handle the response
